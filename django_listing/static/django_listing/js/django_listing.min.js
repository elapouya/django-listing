...
function djlst_load_listing_url(a,b,c){var d=djlst_extract_params_from_url(b,["variation"]);0<Object.keys(d).length&&(c=c?Object.assign({},c,d):d);null===b&&(b=djlst_get_requested_url(a));$("div.django-listing-ajax").length||(window.location.href=b);const e=a.closest("div.django-listing-ajax");e.addClass("spinning");d=$(e).attr("id");const f=$(`form[listing-id=${d}]`),g=$(e).attr("listing-suffix");let h=a.attr("listing-target");h||(h="#"+d);let k=a.attr("listing-part");k||(k="all");d={listing_id:d,
listing_suffix:g,listing_part:k};d=djlst_add_filter_request_data(e,a,d);c&&(d=Object.assign({},d,c));update_csrf_token();$.ajax({type:"POST",url:b,data:d,success:function(l){l.filters_form&&(f.replaceWith(l.filters_form),e.removeClass("spinning"));l.listing&&(e.replaceWith(l.listing),djlst_listing_on_load(),$(document).trigger("djlst_ajax_loaded",{listing_target:h,response:l.listing}))},error:function(l){text="An error occured.\n\nIndications :\n\n"+l.responseText;alert(text)}});return!1}
function djlst_load_listing_href(){return djlst_load_listing_url($(this),$(this).attr("href"))}function djlst_load_listing_val(){var a=$(this).val(),b=$(this).attr("name"),c=$(this).closest("div.django-listing-ajax").attr("ajax_url");a=djlst_replaceUrlParam(c,b,a);return djlst_load_listing_url($(this),a)}
function djlst_post_action_button(a){a.preventDefault();a=$(this);const b=a.closest("div.django-listing-ajax"),c=b.attr("ajax_url");b.addClass("spinning");var d=b.attr("id");const e=$(`form[listing-id=${d}]`),f=b.attr("listing-suffix");let g=a.attr("listing-target");g||(g="#"+d);let h=a.attr("listing-part");h||(h="all");d={listing_id:d,listing_suffix:f,listing_part:h,serialized_data:a.closest("form").serialize()};d[$(this).attr("name")]=$(this).val();d=djlst_add_filter_request_data(b,a,d);update_csrf_token();
$.ajax({type:"POST",url:c,data:d,success:function(k){k.filters_form&&(e.replaceWith(k.filters_form),b.removeClass("spinning"));k.listing&&(b.replaceWith(k.listing),djlst_listing_on_load(),$(document).trigger("djlst_ajax_loaded",{listing_target:g,response:k.listing}))},error:function(k){text="An error occured.\n\nIndications :\n\n"+k.responseText;alert(text)}})}var djlst_mass_op_cbs_displayed=!1;
...
function djlst_post_attached_form(a){a.preventDefault();a=$(this);var b=a.val();let c=a.closest("form.listing-form"),d=c.attr("id"),e=$("#"+c.attr("related-listing"));var f=e.find(".row-container.selected");"update_all"==b&&e.find(".row-container").addClass("selected");if("update_all"==b||"update"==b&&1<f.length){var g=c.find("input:checkbox:visible").length;0==g&&(djlst_show_mass_op_cbs(c),djlst_clear_form(c));if(0==c.find("input:checkbox:checked").length||0==g){setTimeout(function(){alert(use_mass_cb_msg)},
...
b={listing_id:h,listing_suffix:k,listing_part:n,action:"attached_form",action_button:b,selected_pks:g,serialized_data:a.closest("form").serialize()};b=djlst_add_filter_request_data(e,a,b);update_csrf_token();$.ajax({type:"POST",url:f,data:b,success:function(m){let p;m.listing&&e.replaceWith(m.listing);m.attached_form&&(c.replaceWith(m.attached_form),djlst_selection_changed_hook(e));m.object_pk&&(p=$("#"+d),p.find('input[name="object_pk"]').val(m.object_pk));$(document).trigger("djlst_ajax_attached_form_loaded",
...
function djlst_multiple_row_select(a){let b=$(this).closest(".row-container"),c=b.parent(),d=b.index(),e=djlst_last_selected_rows_container,f=djlst_last_selected_index,g=b.closest(".django-listing-container").hasClass("selection-multiple-ctrl"),h;h=b.hasClass("selected")?djlst_multiple_row_do_unselect:djlst_multiple_row_do_select;g&&!a.ctrlKey&&b.siblings().removeClass("selected");a.shiftKey&&e[0]===c[0]&&null!=f?(djlst_map_children_range(c,f,d,h),djlst_unselectText()):h(b);djlst_selection_changed_hook($(this));
...
function djlst_get_requested_url(a){a=a.closest("div.django-listing-container").find("input[name='requested_url']");return a.length?a.val():window.location.href}function djlst_reload_listing_from_form(a,b){a=$(a).closest(".listing-form").attr("listing-id");a=$("#"+a);const c=djlst_get_requested_url(a);djlst_load_listing_url(a,c,b)}
function djlst_selection_changed_hook(a){a=a.closest("div.django-listing-selecting");var b=$("#"+a.attr("attached-form-id"));let c=a.find(".row-container.selected").length;0===c?$(".disabled-if-no-selection").addClass("disabled"):$(".disabled-if-no-selection:not(.no-perm)").removeClass("disabled");1<c?(djlst_clear_form(b),djlst_show_mass_op_cbs(b)):djlst_hide_mass_op_cbs(b);if(b=a.attr("selection-menu-id"))b=$("#"+b).find("span.selected_items"),0==c?b.text(b.attr("none")):1==c?b.text(b.attr("one")):
b.text(b.attr("many").replace("{nb}",c));$(document).trigger("djlst_selection_changed",{listing:a,selected_count:c,all_count:a.attr("nb-rows")})}
...
e);else if(d.is("select")){let f=e;Array.isArray(e)&&(f=e[1],e=e[0]);"boolean"===typeof e&&(e=e?"True":"False");e||(e="");0===d.find("option[value='"+e+"']").length&&(d.hasClass("select2-hidden-accessible")&&d.empty(),d.append($("<option>",{value:e,text:f})));d.val(e);d.hasClass("select2-hidden-accessible")&&d.trigger("change")}else d.val(e)});$(document).trigger("djlst_form_filled",{form:a})}
...
{filters_action:"reset"});return!1});$(".django-listing-container").each(function(){djlst_selection_changed_hook($(this))});$(document.body).on("click",".apply-group-by",function(){djlst_load_listing_url($(this),null)});$(document.body).on("click",".remove-group-by",function(){$(this).closest("div.django-listing-ajax").find(".group-by-container select").val("");djlst_load_listing_url($(this),null)});$('[data-toggle="popover"]').popover();var c=0;$(".dropzone").on("dragenter",function(){c++;$(this).addClass("drag-over")});
$(".dropzone").bind("dragleave",function(){c--;0===c&&$(this).removeClass("drag-over")});$(".dropzone").bind("drop",function(){c=0;$(this).removeClass("drag-over")});$(document.body).on("click",".submit-action-form",function(){var d=$(this).val(),e=$(this).closest(".django-listing-container").find(".action-form");e.find(".action-hidden-value").val(d);e.submit()});$(document).on("select2:open",function(d){d=$(d.target).parent().find(".select2-selection").attr("aria-owns");document.querySelector('input[aria-controls="'+
d+'"]')&&(document.querySelector('input[aria-controls="'+d+'"]').focus(),b=!0)});$("form").keyup(function(d){9==d.keyCode&&(d=$(d.target),d.hasClass("select2-selection")?(b||d.closest(".select2").siblings("select").first().select2("open"),b=!0):b=!1)});$(document.body).on("click",".file-generation-button",function(){var d=$(this),e=$(this).siblings("select");if(e.length&&!e.val())return(e=$(this).data("empty-select-msg"))||(e="Please select a value !"),alert(e),!1;e=$(this).closest("form");var f=
e.closest(".django-listing-container");f=djlst_add_filter_request_data(f,d,{});d=djlst_get_requested_url(d).split("?")[1];(d=(new URLSearchParams(d)).get("sort"))&&(f.sort=d);djlst_patch_form_data(e,f);Cookies.set("file_generation","working",{expires:1});$(this).closest("div.django-listing-ajax").addClass("spinning").removeClass("done");djlst_follow_file_generation()});$(".attached-form-container.sticky .attached-form").length&&($(window).on("scroll resize",()=>a(!1)),$(document).on("djlst_selection_changed djlst_form_filled",
