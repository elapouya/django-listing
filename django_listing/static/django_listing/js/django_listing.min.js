...
function djlst_selection_changed_hook(a){a=a.closest("div.django-listing-selecting");var b=$("#"+a.attr("attached-form-id"));let c=a.find(".row-container.selected").length;0===c?$(".selected-count,.disabled-if-no-selection").addClass("disabled"):$(".selected-count:not(.no-perm),.disabled-if-no-selection:not(.no-perm)").removeClass("disabled");1<c?(djlst_clear_form(b),djlst_show_mass_op_cbs(b)):djlst_hide_mass_op_cbs(b);if(b=a.attr("selection-menu-id"))b=$("#"+b).find("span.selected_items"),0==c?b.text(b.attr("none")):
1==c?b.text(b.attr("one")):b.text(b.attr("many").replace("{nb}",c));b=a.attr("nb-rows");const d=`<span class="badge rounded-pill text-bg-dark">${c}</span>`,e=`<span class="badge rounded-pill text-bg-dark">${b}</span>`;$(".attached-form button.selected-count span.button-extra-middle").html(d);$(".attached-form button.all-count span.button-extra-middle").html(e);$(document).trigger("djlst_selection_changed",{listing:a,selected_count:c,all_count:b})}
...
'"]');c.find("button.all-count span.button-extra-middle").html(b);console.log("all_count =",a);"0"===a?c.find("button.all-count").addClass("disabled"):c.find("button.all-count").removeClass("disabled")})}function djlst_follow_file_generation(){"done"!==Cookies.get("file_generation")?setTimeout(djlst_follow_file_generation,300):(Cookies.remove("file_generation"),$(".spinning").removeClass("spinning").addClass("done"))}
