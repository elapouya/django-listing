...
function djlst_fill_form(a,b){$.each(b,function(c,d){console.log(c,d);c=a.find("[data-model-field='"+c+"']");c.is(":input")&&(c.is("input[type='radio']")?c.filter("[value='"+d+"']").prop("checked",!0):c.is("input[type='checkbox']")?c.prop("checked",d):(c.is("select")&&0===c.find("option[value='"+d+"']").length&&c.append($("<option>",{value:d,text:d})),c.val(d)))})}
