...
listing_suffix:g,listing_part:h};(g=a.attr("action"))&&(d.action=g);d=djlst_add_filter_request_data(e,a,d);c&&(d=Object.assign({},d,c));$(document).trigger("djlst_before_load_listing_url",{listing:k,url:b,payload:d});update_csrf_token();$.ajax({type:"POST",url:b,data:d,traditional:!0,success:function(n){n.filters_form&&(f.replaceWith(n.filters_form),e.removeClass("spinning"));n.listing&&(e.replaceWith(n.listing),djlst_listing_on_load(),$(document).trigger("djlst_ajax_loaded",{listing_target:k,response:n.listing}))},
error:function(n){text="An error occured.\n\nIndications :\n\n"+n.responseText;alert(text)}});$(document).trigger("djlst_after_load_listing_url");return!1}function djlst_load_listing_href(){return djlst_load_listing_url($(this),$(this).attr("href"))}function djlst_load_listing_val(){var a=$(this).val(),b=$(this).attr("name"),c=$(this).closest("div.django-listing-ajax").attr("ajax_url");a=djlst_replaceUrlParam(c,b,a);return djlst_load_listing_url($(this),a)}
...
(e=e.val(),Array.isArray(e)?h=e.filter(function(l){return null!=l}):null!=e&&(h=[e])):"checkbox"===k?e.filter(":checked").each(function(){var l=$(this).val();h.push(null==l||""===l?"on":l)}):"radio"===k?(e=e.filter(":checked").first(),e.length&&(h=[e.val()])):(e=g.val(),null!=e&&(h=[e]));e=f.first();g=(e.attr("type")||"").toLowerCase();if("select"===(e[0]&&e[0].tagName||"").toLowerCase())e.prop("multiple")?f.val(h):f.val(h.length?h[0]:""),f.trigger("change");else if("checkbox"===g){var n=new Set(h.map(String));
f.each(function(){var l=$(this),p=l.val();if(null==p||""===p)p="on";l.prop("checked",n.has(String(p)))}).trigger("change")}else if("radio"===g){var m=h.length?String(h[0]):null;f.each(function(){var l=$(this);l.prop("checked",null!==m&&String(l.val())===m)}).trigger("change")}else f.val(h.length?h[0]:"").trigger("change")}}})}}}
function djlst_post_attached_form(a){const b=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a.preventDefault();const c=$(b),d=c.closest("form").find(".form-fields");var e=c.val();const f=c.closest("form.listing-form"),g=f.closest(".attached-form-container");let k=f.attr("id"),h=$("#"+f.attr("related-listing"));var n=h.find(".row-container.selected");!c.hasClass("flip")&&g.hasClass("flipped")&&(d.addClass("flip-out"),f.hasClass("animate")&&(yield djlst_sleep(300)));var m=c.hasClass("mass-cb-required");
("update_all"==e||m)&&h.find(".row-container").addClass("selected");if("update_all"==e||m||"update"==e&&1<n.length)if(m=f.find("input.mass-op-cb:visible").length,0==m&&(djlst_show_mass_op_cbs(f),djlst_clear_form(f)),0==f.find("input.mass-op-cb:checked").length||0==m){setTimeout(function(){alert(use_mass_cb_msg)},50);return}"clear"==e&&djlst_listing_unselect_all(h);if(m=c.attr("confirm-msg")){var l=c.attr("confirm-msg-nb-items");l=parseInt(l);isNaN(l)&&(l=0);m=m.replace("{nb_items}",n.length);var p=
h.attr("nb-rows");p||(p="");m=m.replace("{nb_all_items}",p);if(n.length>=l&&!confirm(m))return}djlst_copy_f_fields_to_attached_form(f);c.hasClass("flip")&&(d.addClass("flip-out"),g.addClass("flipped"),f.hasClass("animate")&&(yield djlst_sleep(300)));m=n.map(function(){return $(this).attr("data-pk")}).get().join(",");n=h.attr("ajax_url");let u=h.attr("id");l=h.attr("listing-suffix");let t=c.attr("listing-target");t||(t="#"+u);(p=c.attr("listing-part"))||(p="all");e={listing_id:u,listing_suffix:l,listing_part:p,
action:"attached_form",action_button:e,selected_pks:m,serialized_data:c.closest("form").serialize()};e=djlst_add_filter_request_data(h,c,e);$(document).trigger("djlst_before_attached_form_post",{listing:t,form:f,payload:e,nav_obj:c});update_csrf_token();$.ajax({type:"POST",url:n,data:e,traditional:!0,success:function(q){let r;q.listing&&(h.replaceWith(q.listing),h=$("#"+u),djlst_listing_on_load());q.attached_form&&(f.replaceWith(q.attached_form),djlst_selection_changed_hook(h),g.hasClass("flipped")&&
(r=$("#"+k),r.find(".mass-op-cb").hide(),r.removeClass("mass-op-cbs-displayed"),djlst_mass_op_cbs_displayed=!1));q.object_pk&&(r=$("#"+k),r.find('input[name="object_pk"]').val(q.object_pk));$(document).trigger("djlst_ajax_attached_form_loaded",{listing:t,form:r,response:q});c.hasClass("flip")&&""!==q.layout_name||!g.hasClass("flipped")||setTimeout(()=>{g.removeClass("flipped");d.removeClass("flip-out")},300)},error:function(q){text="An error occured.\n\nIndications :\n\n"+q.responseText;alert(text)}});
$(document).trigger("djlst_after_attached_form_post",{listing:t,form:f,payload:e,nav_obj:c})})}var djlst_last_selected_rows_container=null,djlst_last_selected_index=null;
