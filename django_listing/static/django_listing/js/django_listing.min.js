...
function djlst_removeUrlParam(a,b){if(!a.startsWith("/")||!b||!b.length)return a;const [c,d]=a.split("?");if(!d)return a;const e=new URLSearchParams(d);b.forEach(f=>e.delete(f));return(a=e.toString())?`${c}?${a}`:c}function update_csrf_token(){$.ajaxSetup({data:{csrfmiddlewaretoken:Cookies.get("csrftoken")}})}function djlst_get_form_data_advanced(a){a=new FormData(a);let b={};for(const [c,d]of a.entries())b[c]||(b[c]=1<a.getAll(c).length?a.getAll(c):d);return b}
...
d+'"]').focus(),b=!0)});$("form").keyup(function(d){9==d.keyCode&&(d=$(d.target),d.hasClass("select2-selection")?(b||d.closest(".select2").siblings("select").first().select2("open"),b=!0):b=!1)});$('form.attached-form input[type="text"], form.attached-form input[type="number"]').on("keypress",function(d){if(13===d.which)return d.preventDefault(),$(this).blur().focus(),!1});$(document.body).on("click",".file-generation-button",function(){function d(){"done"!==Cookies.get("file_generation")?setTimeout(d,
