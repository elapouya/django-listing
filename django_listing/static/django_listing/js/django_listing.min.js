...
function djlst_add_filter_request_data(a,b){var c=a.attr("id");c=$(`form[listing-id=${c}]`);if(c.length){const d=c.attr("method");void 0!==d&&"post"==d.toLowerCase()&&(c=Object.fromEntries(new FormData(c[0])),b=Object.assign({},b,c))}c=a.find(".group-by-select");c.length&&(b.gb_cols=c.val().join(","));a=a.find(".annotation-select");a.length&&(b.gb_annotate_cols=a.val().join(","));return b}
function djlst_load_listing_url(a,b){$("div.django-listing-ajax").length||(window.location.href=b);const c=a.closest("div.django-listing-ajax");c.addClass("spinning");var d=$(c).attr("id");const e=$(c).attr("listing-suffix");let f=a.attr("listing-target");f||(f="#"+d);(a=a.attr("listing-part"))||(a="all");d={listing_id:d,listing_suffix:e,listing_part:a};d=djlst_add_filter_request_data(c,d);update_csrf_token();$.ajax({type:"POST",url:b,data:d,success:function(g){$(f).replaceWith(g);djlst_listing_on_load();
$(document).trigger("djlst_ajax_loaded",{listing_target:f,response:g})},error:function(g){text="An error occured.\n\nIndications :\n\n"+g.responseText;alert(text)}});return!1}function djlst_load_listing_href(){return djlst_load_listing_url($(this),$(this).attr("href"))}function djlst_load_listing_val(){var a=$(this).val(),b=$(this).attr("name"),c=$(this).closest("div.django-listing-ajax").attr("ajax_url");a=djlst_replaceUrlParam(c,b,a);return djlst_load_listing_url($(this),a)}
function djlst_post_action_button(a){a.preventDefault();var b=$(this);a=b.closest("div.django-listing-ajax");var c=a.attr("ajax_url");a.addClass("spinning");var d=a.attr("id"),e=a.attr("listing-suffix"),f=b.attr("listing-target");f||(f="#"+d);var g=b.attr("listing-part");g||(g="all");b={listing_id:d,listing_suffix:e,listing_part:g,serialized_data:b.closest("form").serialize()};b[$(this).attr("name")]=$(this).val();b=djlst_add_filter_request_data(a,b);update_csrf_token();$.ajax({type:"POST",url:c,
data:b,success:function(h){$(f).replaceWith(h);djlst_listing_on_load();$(document).trigger("djlst_ajax_loaded",{listing_target:f,response:h})},error:function(h){text="An error occured.\n\nIndications :\n\n"+h.responseText;alert(text)}})}var djlst_mass_op_cbs_displayed=!1;function djlst_show_mass_op_cbs(a){djlst_mass_op_cbs_displayed=!1;a.find(".mass-op-cb").prop("checked",!1).show();a.addClass("mass-op-cbs-displayed");setTimeout(function(){djlst_mass_op_cbs_displayed=!0},100)}
...
function djlst_post_attached_form(a){a.preventDefault();a=$(this);let b=a.val(),c=a.closest("form.listing-form"),d=c.attr("id"),e=$("#"+c.attr("related-listing"));var f=e.find(".row-container.selected");"update_all"==b&&e.find(".row-container").addClass("selected");if("update_all"==b||"update"==b&&1<f.length){var g=c.find("input:checkbox:visible").length;0==g&&(djlst_show_mass_op_cbs(c),djlst_clear_form(c));if(0==c.find("input:checkbox:checked").length||0==g){setTimeout(function(){alert(use_mass_cb_msg)},
...
a={listing_id:h,listing_suffix:l,listing_part:n,action:"attached_form",action_button:b,selected_pks:g,serialized_data:a.closest("form").serialize()};a=djlst_add_filter_request_data(e,a);update_csrf_token();$.ajax({type:"POST",url:f,data:a,success:function(k){let p;k.listing&&e.replaceWith(k.listing);k.attached_form&&(c.replaceWith(k.attached_form),djlst_selection_changed_hook(e));k.object_pk&&(p=$("#"+d),p.find('input[name="object_pk"]').val(k.object_pk));$(document).trigger("djlst_ajax_attached_form_loaded",
...
function djlst_reload_listing_from_form(a){a=$(a).closest(".listing-form").attr("listing-id");a=$("#"+a);djlst_load_listing_url(a,window.location.href)}
...
"Available columns",selectedTitle:a.attr("selected-title")||"Selected columns"})});$(".django-listing-container.format-numbers td.type-Decimal,.django-listing-container.format-numbers td.type-int,.django-listing-container.format-numbers td.type-float").djlst_format_digits()}function djlst_follow_file_generation(){"done"!==Cookies.get("file_generation")?setTimeout(djlst_follow_file_generation,300):(Cookies.remove("file_generation"),$(".spinning").removeClass("spinning").addClass("done"))}
function djlst_attached_form_input_changed(a){djlst_mass_op_cbs_displayed&&$(this).closest(".form-field").find(".mass-op-cb:visible").prop("checked",!0)}function djlst_reset_form_fields(a){a[0].reset();a.find("select").each(function(){$(this).hasClass("select2-hidden-accessible")&&$(this).val(null).trigger("change")})}
...
return!1});$(".django-listing-container").each(function(){djlst_selection_changed_hook($(this))});$(document.body).on("click",".apply-group-by",function(){djlst_load_listing_url($(this),window.location.href)});$(document.body).on("click",".remove-group-by",function(){$(this).closest("div.django-listing-ajax").find(".group-by-container select").val("");djlst_load_listing_url($(this),window.location.href)});$('[data-toggle="popover"]').popover();var c=0;$(".dropzone").on("dragenter",function(){c++;
$(this).addClass("drag-over")});$(".dropzone").bind("dragleave",function(){c--;0===c&&$(this).removeClass("drag-over")});$(".dropzone").bind("drop",function(){c=0;$(this).removeClass("drag-over")});$(document.body).on("click",".submit-action-form",function(){var d=$(this).val(),e=$(this).closest(".django-listing-container").find(".action-form");e.find(".action-hidden-value").val(d);e.submit()});$(document).on("select2:open",function(d){d=$(d.target).parent().find(".select2-selection").attr("aria-owns");
document.querySelector('input[aria-controls="'+d+'"]')&&(document.querySelector('input[aria-controls="'+d+'"]').focus(),b=!0)});$("form").keyup(function(d){9==d.keyCode&&(d=$(d.target),d.hasClass("select2-selection")?(b||d.closest(".select2").siblings("select").first().select2("open"),b=!0):b=!1)});$(document.body).on("click",".file-generation-button",function(){var d=$(this).siblings("select");if(d.length&&!d.val())return(d=$(this).data("empty-select-msg"))||(d="Please select a value !"),alert(d),
!1;d=$(this).closest("form");var e=d.closest(".django-listing-container");e=djlst_add_filter_request_data(e,{});djlst_patch_form_data(d,e);Cookies.set("file_generation","working",{expires:1});$(this).closest("div.django-listing-ajax").addClass("spinning").removeClass("done");djlst_follow_file_generation()});$(".attached-form-container.sticky .attached-form").length&&($(window).on("scroll resize",()=>a(!1)),$(document).on("djlst_selection_changed djlst_form_filled",()=>a(!0)));djlst_listing_on_load()});
