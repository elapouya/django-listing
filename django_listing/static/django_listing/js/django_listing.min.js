...
$.fn.djlst_format_digits=function(){return this.each(function(){const a=$(this),b=a.text().trim().replace(/\B(?=(\d{3})+(?!\d))/g," ");a.text(b)})};function djlst_replaceUrlParam(a,b,c){null==c&&(c="");a=a.startsWith("/")?new URL(a,window.location.origin):new URL(a);a.searchParams.set(b,c);return a.toString()}function djlst_removeUrlParam(a,b){a=new URL(a);a.searchParams.delete(b);return a.toString()}function update_csrf_token(){$.ajaxSetup({data:{csrfmiddlewaretoken:Cookies.get("csrftoken")}})}
