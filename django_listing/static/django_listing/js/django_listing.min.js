...
a)})}function djlst_follow_file_generation(){"done"!==Cookies.get("file_generation")?setTimeout(djlst_follow_file_generation,300):(Cookies.remove("file_generation"),$(".spinning").removeClass("spinning").addClass("done"))}function djlst_attached_form_input_changed(a){djlst_mass_op_cbs_displayed&&$(this).closest(".form-field").find(".mass-op-cb:visible").prop("checked",!0)}
...
djlst_follow_file_generation()});$(".attached-form-container.sticky .attached-form").length&&($(window).on("scroll resize",()=>a(!1)),$(document).on("djlst_selection_changed djlst_form_filled",()=>a(!0)));djlst_listing_on_load()});$.fn.djlst_format_digits=function(){return this.each(function(){const a=$(this),b=a.text().trim().replace(/\B(?=(\d{3})+(?!\d))/g," ");a.text(b)})};
