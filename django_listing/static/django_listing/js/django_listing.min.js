...
function djlst_load_listing_url(a,b){null===b&&(b=djlst_get_requested_url(a));$("div.django-listing-ajax").length||(window.location.href=b);const c=a.closest("div.django-listing-ajax");c.addClass("spinning");var d=$(c).attr("id");const e=$(`form[listing-id=${d}]`),f=$(c).attr("listing-suffix");let g=a.attr("listing-target");g||(g="#"+d);let k=a.attr("listing-part");k||(k="all");d={listing_id:d,listing_suffix:f,listing_part:k};d=djlst_add_filter_request_data(c,a,d);update_csrf_token();$.ajax({type:"POST",
url:b,data:d,success:function(h){console.log(h);h.filters_form&&(e.replaceWith(h.filters_form),c.removeClass("spinning"));h.listing&&(c.replaceWith(h.listing),djlst_listing_on_load(),$(document).trigger("djlst_ajax_loaded",{listing_target:g,response:h.listing}))},error:function(h){text="An error occured.\n\nIndications :\n\n"+h.responseText;alert(text)}});return!1}function djlst_load_listing_href(){return djlst_load_listing_url($(this),$(this).attr("href"))}
...
$.ajax({type:"POST",url:c,data:d,success:function(h){h.filters_form&&(e.replaceWith(h.filters_form),b.removeClass("spinning"));h.listing&&(b.replaceWith(h.listing),djlst_listing_on_load(),$(document).trigger("djlst_ajax_loaded",{listing_target:g,response:h.listing}))},error:function(h){text="An error occured.\n\nIndications :\n\n"+h.responseText;alert(text)}})}var djlst_mass_op_cbs_displayed=!1;
...
50);return}}"clear"==b&&djlst_listing_unselect_all(e);if(g=a.attr("confirm-msg")){var k=a.attr("confirm-msg-nb-items");k=parseInt(k);isNaN(k)&&(k=0);g=g.replace("{nb_items}",f.length);var h=e.attr("nb-rows");h||(h="");g=g.replace("{nb_all_items}",h);if(f.length>=k&&!confirm(g))return}g=f.map(function(){return $(this).attr("data-pk")}).get().join(",");f=e.attr("ajax_url");k=e.attr("id");h=e.attr("listing-suffix");let m=a.attr("listing-target");m||(m="#"+k);let n=a.attr("listing-part");n||(n="all");
b={listing_id:k,listing_suffix:h,listing_part:n,action:"attached_form",action_button:b,selected_pks:g,serialized_data:a.closest("form").serialize()};b=djlst_add_filter_request_data(e,a,b);update_csrf_token();$.ajax({type:"POST",url:f,data:b,success:function(l){let p;l.listing&&e.replaceWith(l.listing);l.attached_form&&(c.replaceWith(l.attached_form),djlst_selection_changed_hook(e));l.object_pk&&(p=$("#"+d),p.find('input[name="object_pk"]').val(l.object_pk));$(document).trigger("djlst_ajax_attached_form_loaded",
{listing:m,form:p,response:l})},error:function(l){text="An error occured.\n\nIndications :\n\n"+l.responseText;alert(text)}})}var djlst_last_selected_rows_container=null,djlst_last_selected_index=null;
