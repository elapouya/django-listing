...
listing_suffix:g,listing_part:h};(g=a.attr("action"))&&(d.action=g);d=djlst_add_filter_request_data(e,a,d);c&&(d=Object.assign({},d,c));$(document).trigger("djlst_before_load_listing_url",{listing:k,url:b,payload:d});update_csrf_token();$.ajax({type:"POST",url:b,data:d,traditional:!0,success:function(m){m.filters_form&&(f.replaceWith(m.filters_form),e.removeClass("spinning"));m.listing&&(e.replaceWith(m.listing),djlst_listing_on_load(),$(document).trigger("djlst_ajax_loaded",{listing_target:k,response:m.listing}))},
error:function(m){text="An error occured.\n\nIndications :\n\n"+m.responseText;alert(text)}});$(document).trigger("djlst_after_load_listing_url");return!1}function djlst_load_listing_href(){return djlst_load_listing_url($(this),$(this).attr("href"))}function djlst_load_listing_val(){var a=$(this).val(),b=$(this).attr("name"),c=$(this).closest("div.django-listing-ajax").attr("ajax_url");a=djlst_replaceUrlParam(c,b,a);return djlst_load_listing_url($(this),a)}
...
(e=e.val(),Array.isArray(e)?h=e.filter(function(l){return null!=l}):null!=e&&(h=[e])):"checkbox"===k?e.filter(":checked").each(function(){var l=$(this).val();h.push(null==l||""===l?"on":l)}):"radio"===k?(e=e.filter(":checked").first(),e.length&&(h=[e.val()])):(e=g.val(),null!=e&&(h=[e]));e=f.first();g=(e.attr("type")||"").toLowerCase();if("select"===(e[0]&&e[0].tagName||"").toLowerCase())e.prop("multiple")?f.val(h):f.val(h.length?h[0]:""),f.trigger("change");else if("checkbox"===g){var m=new Set(h.map(String));
f.each(function(){var l=$(this),p=l.val();if(null==p||""===p)p="on";l.prop("checked",m.has(String(p)))}).trigger("change")}else if("radio"===g){var n=h.length?String(h[0]):null;f.each(function(){var l=$(this);l.prop("checked",null!==n&&String(l.val())===n)}).trigger("change")}else f.val(h.length?h[0]:"").trigger("change")}}})}}}
function djlst_post_attached_form(a){const b=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a.preventDefault();const c=$(b),d=c.closest("form").find(".form-fields");var e=c.val();const f=c.closest("form.listing-form"),g=f.closest(".attached-form-container");let k=f.attr("id"),h=$("#"+f.attr("related-listing"));var m=h.find(".row-container.selected");!c.hasClass("flip")&&g.hasClass("flipped")&&(d.addClass("flip-out"),f.hasClass("animate")&&(yield djlst_sleep(300)));"update_all"==
e&&h.find(".row-container").addClass("selected");if("update_all"==e||"update"==e&&1<m.length){var n=f.find("input.mass-op-cb:visible").length;0==n&&(djlst_show_mass_op_cbs(f),djlst_clear_form(f));if(0==f.find("input.mass-op-cb:checked").length||0==n){setTimeout(function(){alert(use_mass_cb_msg)},50);return}}"clear"==e&&djlst_listing_unselect_all(h);if(n=c.attr("confirm-msg")){var l=c.attr("confirm-msg-nb-items");l=parseInt(l);isNaN(l)&&(l=0);n=n.replace("{nb_items}",m.length);var p=h.attr("nb-rows");
p||(p="");n=n.replace("{nb_all_items}",p);if(m.length>=l&&!confirm(n))return}djlst_copy_f_fields_to_attached_form(f);c.hasClass("flip")&&(d.addClass("flip-out"),g.addClass("flipped"),f.hasClass("animate")&&(yield djlst_sleep(300)));n=m.map(function(){return $(this).attr("data-pk")}).get().join(",");m=h.attr("ajax_url");let u=h.attr("id");l=h.attr("listing-suffix");let t=c.attr("listing-target");t||(t="#"+u);(p=c.attr("listing-part"))||(p="all");e={listing_id:u,listing_suffix:l,listing_part:p,action:"attached_form",
action_button:e,selected_pks:n,serialized_data:c.closest("form").serialize()};e=djlst_add_filter_request_data(h,c,e);$(document).trigger("djlst_before_attached_form_post",{listing:t,form:f,payload:e,nav_obj:c});update_csrf_token();$.ajax({type:"POST",url:m,data:e,traditional:!0,success:function(q){let r;q.listing&&(h.replaceWith(q.listing),h=$("#"+u),djlst_listing_on_load());q.attached_form&&(f.replaceWith(q.attached_form),djlst_selection_changed_hook(h),g.hasClass("flipped")&&(r=$("#"+k),r.find(".mass-op-cb").hide(),
r.removeClass("mass-op-cbs-displayed"),djlst_mass_op_cbs_displayed=!1));q.object_pk&&(r=$("#"+k),r.find('input[name="object_pk"]').val(q.object_pk));$(document).trigger("djlst_ajax_attached_form_loaded",{listing:t,form:r,response:q});c.hasClass("flip")&&""!==q.layout_name||!g.hasClass("flipped")||setTimeout(()=>{g.removeClass("flipped");d.removeClass("flip-out")},300)},error:function(q){text="An error occured.\n\nIndications :\n\n"+q.responseText;alert(text)}});$(document).trigger("djlst_after_attached_form_post",
{listing:t,form:f,payload:e,nav_obj:c})})}var djlst_last_selected_rows_container=null,djlst_last_selected_index=null;
