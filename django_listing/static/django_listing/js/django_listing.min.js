...
{listing:t,form:q,response:n});c.hasClass("flip")&&""!==n.layout_name||!g.hasClass("flipped")||setTimeout(()=>{g.removeClass("flipped");d.removeClass("flip-out")},300)},error:function(n){text="An error occured.\n\nIndications :\n\n"+n.responseText;alert(text)}})})}var djlst_last_selected_rows_container=null,djlst_last_selected_index=null;
...
function djlst_selection_changed_hook(a){a=a.closest("div.django-listing-selecting");const b=$('body form[related-listing="'+a.attr("id")+'"]');var c=$("#"+a.attr("attached-form-id"));let d=a.find(".row-container.selected").length;0===d?($(".selected-count,.disabled-if-no-selection").addClass("disabled"),b.find("button.hide-if-no-selection").hide(),b.find("button.hide-if-selection").show()):($(".selected-count:not(.no-perm),.disabled-if-no-selection:not(.no-perm)").removeClass("disabled"),b.find("button.hide-if-no-selection").show(),
b.find("button.hide-if-selection").hide());1<d?(djlst_clear_form(c),djlst_show_mass_op_cbs(c)):djlst_hide_mass_op_cbs(c);if(c=a.attr("selection-menu-id"))c=$("#"+c).find("span.selected_items"),0==d?c.text(c.attr("none")):1==d?c.text(c.attr("one")):c.text(c.attr("many").replace("{nb}",d));c=a.attr("nb-rows");const e=`<span class="badge rounded-pill text-bg-dark">${d}</span>`,f=`<span class="badge rounded-pill text-bg-dark">${c}</span>`;b.find("button.selected-count span.button-extra-middle").html(e);
b.find("button.all-count span.button-extra-middle").html(f);$(document).trigger("djlst_selection_changed",{listing:a,selected_count:d,all_count:c})}
...
"Available columns",selectedTitle:a.attr("selected-title")||"Selected columns"})});$(".django-listing-container.format-numbers .type-Decimal,.django-listing-container.format-numbers .type-int,.django-listing-container.format-numbers .type-float,.django-listing-container.format-numbers .format-number").djlst_format_digits();$(".django-listing-container").each(function(){const a=$(this).attr("nb-rows"),b=`<span class="badge rounded-pill text-bg-dark">${a}</span>`,c=$('body form[related-listing="'+this.id+
'"]');c.find("button.all-count span.button-extra-middle").html(b);"0"===a?c.find("button.all-count").addClass("disabled"):c.find("button.all-count").removeClass("disabled");c.find("button.hide-if-no-selection").hide();c.find("button.hide-if-selection").show()})}function djlst_follow_file_generation(){"done"!==Cookies.get("file_generation")?setTimeout(djlst_follow_file_generation,300):(Cookies.remove("file_generation"),$(".spinning").removeClass("spinning").addClass("done"))}
