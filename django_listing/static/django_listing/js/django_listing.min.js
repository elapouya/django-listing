...
if("update_all"==e||m||"update"==e&&1<n.length)if(m=f.find("input.mass-op-cb:visible").length,0==m&&(djlst_show_mass_op_cbs(f),djlst_clear_form(f)),0==f.find("input.mass-op-cb:checked").length||0==m){setTimeout(function(){alert(use_mass_cb_msg)},50);return}"clear"==e&&djlst_listing_unselect_all(h);if(m=c.attr("confirm-msg")){var l=c.attr("confirm-msg-nb-items");l=parseInt(l);isNaN(l)&&(l=0);m=m.replace("{nb_items}",n.length);var p=h.attr("nb-rows");p||(p="");m=m.replace("{nb_all_items}",p);if(n.length>=
l&&!confirm(m))return}djlst_copy_f_fields_to_attached_form(f);c.hasClass("flip")&&(d.addClass("flip-out"),g.addClass("flipped"),f.hasClass("animate")&&(yield djlst_sleep(300)));m=n.map(function(){return $(this).attr("data-pk")}).get().join(",");n=h.attr("ajax_url");let u=h.attr("id");l=h.attr("listing-suffix");let t=c.attr("listing-target");t||(t="#"+u);(p=c.attr("listing-part"))||(p="all");e={listing_id:u,listing_suffix:l,listing_part:p,action:"attached_form",action_button:e,selected_pks:m,serialized_data:c.closest("form").serialize()};
e=djlst_add_filter_request_data(h,c,e);$(document).trigger("djlst_before_attached_form_post",{listing:t,form:f,payload:e,nav_obj:c});update_csrf_token();$.ajax({type:"POST",url:n,data:e,traditional:!0,success:function(q){let r;q.listing&&(h.replaceWith(q.listing),h=$("#"+u),djlst_listing_on_load());q.attached_form&&(f.replaceWith(q.attached_form),djlst_selection_changed_hook(h),g.hasClass("flipped")&&(r=$("#"+k),r.find(".mass-op-cb").hide(),r.removeClass("mass-op-cbs-displayed"),djlst_mass_op_cbs_displayed=
!1));q.object_pk&&(r=$("#"+k),r.find('input[name="object_pk"]').val(q.object_pk));$(document).trigger("djlst_ajax_attached_form_loaded",{listing:t,form:r,response:q});c.hasClass("flip")&&""!==q.layout_name||!g.hasClass("flipped")||setTimeout(()=>{g.removeClass("flipped");d.removeClass("flip-out")},300)},error:function(q){text="An error occured.\n\nIndications :\n\n"+q.responseText;alert(text)}});$(document).trigger("djlst_after_attached_form_post",{listing:t,form:f,payload:e,nav_obj:c})})}
