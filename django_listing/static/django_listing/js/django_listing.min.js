...
$jscomp.polyfill("Array.prototype.includes",function(a){return a?a:function(b,c){var d=this;d instanceof String&&(d=String(d));var e=d.length;c=c||0;for(0>c&&(c=Math.max(c+e,0));c<e;c++){var f=d[c];if(f===b||Object.is(f,b))return!0}return!1}},"es7","es3");$.fn.djlst_format_digits=function(){return this.each(function(){const a=$(this),b=a.text().trim().replace(/\B(?=(\d{3})+(?!\d))/g," ");a.text(b)})};
...
function djlst_add_filter_request_data(a,b){a=$(`form[listing-id=${a}]`);if(a.length){const c=a.attr("method");void 0!==c&&"post"==c.toLowerCase()&&(a=Object.fromEntries(new FormData(a[0])),b=Object.assign({},b,a))}return b}
function djlst_load_listing_url(a,b){$("div.django-listing-ajax").length||(window.location.href=b);var c=a.closest("div.django-listing-ajax");c.addClass("spinning");var d=$(c).attr("id"),e=$(c).attr("listing-suffix");let f=a.attr("listing-target");f||(f="#"+d);(a=a.attr("listing-part"))||(a="all");e={listing_id:d,listing_suffix:e,listing_part:a};e=djlst_add_filter_request_data(d,e);d=c.find(".group-by-select");d.length&&(e.gb_cols=d.val().join(","));c=c.find(".annotation-select");c.length&&(e.gb_annotate_cols=
c.val().join(","));console.log(e);update_csrf_token();$.ajax({type:"POST",url:b,data:e,success:function(g){$(f).replaceWith(g);djlst_listing_on_load();$(document).trigger("djlst_ajax_loaded",{listing_target:f,response:g})},error:function(g){text="An error occured.\n\nIndications :\n\n"+g.responseText;alert(text)}});return!1}function djlst_load_listing_href(){return djlst_load_listing_url($(this),$(this).attr("href"))}
...
document.querySelector('input[aria-controls="'+d+'"]')&&(document.querySelector('input[aria-controls="'+d+'"]').focus(),b=!0)});$("form").keyup(function(d){9==d.keyCode&&(d=$(d.target),d.hasClass("select2-selection")?(b||d.closest(".select2").siblings("select").first().select2("open"),b=!0):b=!1)});$(".file-generation-button").on("click",function(){var d=$(this).siblings("select");if(d.length&&!d.val())return(d=$(this).data("empty-select-msg"))||(d="Please select a value !"),alert(d),!1;Cookies.set("file_generation",
"working",{expires:1});$(this).closest("div.django-listing-ajax").addClass("spinning").removeClass("done");djlst_follow_file_generation()});$(".attached-form-container.sticky .attached-form").length&&($(window).on("scroll resize",()=>a(!1)),$(document).on("djlst_selection_changed djlst_form_filled",()=>a(!0)));djlst_listing_on_load()});
