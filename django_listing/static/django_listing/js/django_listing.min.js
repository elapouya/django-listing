...
function djlst_format_number(a){return String(a).replace(/\B(?=(\d{3})+(?!\d))/g," ")}const djlst_sleep=a=>new Promise(b=>setTimeout(b,a));$.fn.djlst_format_digits=function(){return this.each(function(){const a=$(this),b=a.text().trim().replace(/\B(?=(\d{3})+(?!\d))/g," ");a.text(b)})};function djlst_replaceUrlParam(a,b,c){null==c&&(c="");a=a.startsWith("/")?new URL(a,window.location.origin):new URL(a);a.searchParams.set(b,c);return a.toString()}
function djlst_removeUrlParam(a,b){if(!a.startsWith("/")||!b||!b.length)return a;const [c,d]=a.split("?");if(!d)return a;const e=new URLSearchParams(d);b.forEach(f=>e.delete(f));return(a=e.toString())?`${c}?${a}`:c}function update_csrf_token(){$.ajaxSetup({data:{csrfmiddlewaretoken:Cookies.get("csrftoken")}})}
...
function djlst_post_attached_form(a){const b=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a.preventDefault();const c=$(b),d=c.closest("form").find(".form-fields");var e=c.val();const f=c.closest("form.listing-form"),g=f.closest(".attached-form-container");let k=f.attr("id"),h=$("#"+f.attr("related-listing"));var l=h.find(".row-container.selected");!c.hasClass("flip")&&g.hasClass("flipped")&&(d.removeClass("flip-in").addClass("flip-out"),yield djlst_sleep(300));"update_all"==
e&&h.find(".row-container").addClass("selected");if("update_all"==e||"update"==e&&1<l.length){var m=f.find("input.mass-op-cb:visible").length;0==m&&(djlst_show_mass_op_cbs(f),djlst_clear_form(f));if(0==f.find("input.mass-op-cb:checked").length||0==m){setTimeout(function(){alert(use_mass_cb_msg)},50);return}}"clear"==e&&djlst_listing_unselect_all(h);if(m=c.attr("confirm-msg")){var p=c.attr("confirm-msg-nb-items");p=parseInt(p);isNaN(p)&&(p=0);m=m.replace("{nb_items}",l.length);var r=h.attr("nb-rows");
r||(r="");m=m.replace("{nb_all_items}",r);if(l.length>=p&&!confirm(m))return}c.hasClass("flip")&&(d.addClass("flip-out"),g.addClass("flipped"),yield djlst_sleep(300));m=l.map(function(){return $(this).attr("data-pk")}).get().join(",");l=h.attr("ajax_url");p=h.attr("id");r=h.attr("listing-suffix");let t=c.attr("listing-target");t||(t="#"+p);let u=c.attr("listing-part");u||(u="all");e={listing_id:p,listing_suffix:r,listing_part:u,action:"attached_form",action_button:e,selected_pks:m,serialized_data:c.closest("form").serialize()};
e=djlst_add_filter_request_data(h,c,e);update_csrf_token();$.ajax({type:"POST",url:l,data:e,success:function(n){let q;n.listing&&h.replaceWith(n.listing);n.attached_form&&(f.replaceWith(n.attached_form),djlst_selection_changed_hook(h),g.hasClass("flipped")&&(q=$("#"+k),q.find(".mass-op-cb").hide(),q.removeClass("mass-op-cbs-displayed"),djlst_mass_op_cbs_displayed=!1));n.object_pk&&(q=$("#"+k),q.find('input[name="object_pk"]').val(n.object_pk));$(document).trigger("djlst_ajax_attached_form_loaded",
{listing:t,form:q,response:n});!c.hasClass("flip")&&g.hasClass("flipped")&&setTimeout(()=>{g.removeClass("flipped");d.removeClass("flip-out")},300)},error:function(n){text="An error occured.\n\nIndications :\n\n"+n.responseText;alert(text)}})})}var djlst_last_selected_rows_container=null,djlst_last_selected_index=null;
...
b.text(b.attr("many").replace("{nb}",c));b=a.attr("nb-rows");const d=`<span class="badge rounded-pill text-bg-dark">${c}</span>`,e=`<span class="badge rounded-pill text-bg-dark">${b}</span>`;$(".attached-form button.selected-count span.button-extra-middle").html(d);$(".attached-form button.all-count span.button-extra-middle").html(e);$(document).trigger("djlst_selection_changed",{listing:a,selected_count:c,all_count:b})}
...
$(document.body).on("click",".apply-group-by",function(){djlst_load_listing_url($(this),null)});$(document.body).on("click",".remove-group-by",function(){$(this).closest("div.django-listing-ajax").find(".group-by-container select").val("");let d=djlst_get_requested_url($(this));d=djlst_removeUrlParam(d,["sort","page","per_page"]);djlst_load_listing_url($(this),d)});$('[data-toggle="popover"]').popover();var c=0;$(".dropzone").on("dragenter",function(){c++;$(this).addClass("drag-over")});$(".dropzone").bind("dragleave",
function(){c--;0===c&&$(this).removeClass("drag-over")});$(".dropzone").bind("drop",function(){c=0;$(this).removeClass("drag-over")});$(document.body).on("click",".submit-action-form",function(){var d=$(this).val(),e=$(this).closest(".django-listing-container").find(".action-form");e.find(".action-hidden-value").val(d);e.submit()});$(document).on("select2:open",function(d){d=$(d.target).parent().find(".select2-selection").attr("aria-owns");document.querySelector('input[aria-controls="'+d+'"]')&&(document.querySelector('input[aria-controls="'+
d+'"]').focus(),b=!0)});$("form").keyup(function(d){9==d.keyCode&&(d=$(d.target),d.hasClass("select2-selection")?(b||d.closest(".select2").siblings("select").first().select2("open"),b=!0):b=!1)});$(document.body).on("click",".file-generation-button",function(){var d=$(this),e=$(this).siblings("select");if(e.length&&!e.val())return(e=$(this).data("empty-select-msg"))||(e="Please select a value !"),alert(e),!1;e=$(this).closest("form");var f=e.closest(".django-listing-container");f=djlst_add_filter_request_data(f,
d,{});d=djlst_get_requested_url(d).split("?")[1];(d=(new URLSearchParams(d)).get("sort"))&&(f.sort=d);djlst_patch_form_data(e,f);Cookies.set("file_generation","working",{expires:1});$(this).closest("div.django-listing-ajax").addClass("spinning").removeClass("done");djlst_follow_file_generation()});$(".attached-form-container.sticky .attached-form").length&&($(window).on("scroll resize",()=>a(!1)),$(document).on("djlst_selection_changed djlst_form_filled",()=>a(!0)));djlst_listing_on_load()});
