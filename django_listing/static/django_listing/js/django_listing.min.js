...
function djlst_load_listing_url(a,b){$("div.django-listing-ajax").length||(window.location.href=b);var c=a.closest("div.django-listing-ajax");c.addClass("spinning");var d=$(c).attr("id"),e=$(c).attr("listing-suffix"),f=a.attr("listing-target");f||(f="#"+d);(a=a.attr("listing-part"))||(a="all");update_csrf_token();$(f).load(b,{listing_id:d,listing_suffix:e,listing_part:a},function(g,h,k){"error"==h?(c.removeClass("spinning"),alert(g)):$(this).children(":first").unwrap();djlst_listing_on_load();$(document).trigger("djlst_ajax_loaded",
[f])});return!1}function djlst_load_listing_href(){return djlst_load_listing_url($(this),$(this).attr("href"))}function djlst_load_listing_val(){var a=$(this).val(),b=$(this).attr("name"),c=$(this).closest("div.django-listing-ajax").attr("ajax_url");a=djlst_replaceUrlParam(c,b,a);return djlst_load_listing_url($(this),a)}
...
success:function(g){$(e).html(g);$(e).children(":first").unwrap();djlst_listing_on_load();$(document).trigger("djlst_ajax_loaded",[e])}})}var djlst_last_selected_rows_container=null,djlst_last_selected_index=null;
function djlst_multiple_row_select(a){let b=$(this).closest(".row-container"),c=b.parent(),d=b.index(),e=djlst_last_selected_rows_container,f=djlst_last_selected_index,g;g=b.hasClass("selected")?djlst_multiple_row_do_unselect:djlst_multiple_row_do_select;a.shiftKey&&e[0]===c[0]&&null!=f?(djlst_map_children_range(c,f,d,g),djlst_unselectText()):g(b);djlst_selection_menu_update($(this));djlst_last_selected_rows_container=c;djlst_last_selected_index=d}
...
function djlst_multiple_row_do_select(a){var b=a.find("input.row-select").first();a.addClass("selected");b.attr("name",b.attr("select-name"));a.find("input.selection-box").first().prop("checked",!0);b=$("#"+a.closest(".django-listing-container").attr("related-form"));b.length&&(a=decodeURIComponent(a.attr("data-serialized-object")),console.log("serialized_obj",a),a=JSON.parse(a),djlst_fill_form(b,a),console.log(a))}
function djlst_unique_row_select(a){a=$(this).closest(".row-container");a.hasClass("selected")?djlst_multiple_row_do_unselect(a):(a.siblings().removeClass("selected").find("input.row-select").removeAttr("name"),djlst_multiple_row_do_select(a));djlst_selection_menu_update($(this))}
...
function djlst_listing_unselect_all(a){a.find(".row-container").removeClass("selected");a.find("input.selection-box").prop("checked",!1);a.find("input.row-select").each(function(){$(this).removeAttr("name")});djlst_selection_menu_update(a)}function djlst_unselectText(){window.getSelection?window.getSelection().removeAllRanges():document.selection&&document.selection.empty()}
...
function djlst_fill_form(a,b){$.each(b.fields,function(c,d){console.log(c,d);var e=a.find("[data-model-field='"+c+"']");e.is(":input")&&(e.is("input[type='radio']")?e.filter("[value='"+d+"']").prop("checked",!0):e.is("input[type='checkbox']")?e.prop("checked",d):(e.is("select")&&0===e.find("option[value='"+d+"']").length&&(e.empty(),b.data?e.append($("<option>",{value:d,text:b.data[c]||d})):e.append($("<option>",{value:d,text:d}))),e.val(d)))})}
