...
djlst_listing_on_load();$(document).trigger("djlst_ajax_loaded",{listing_target:d,response:g})},error:function(g){text="An error occured.\n\nIndications :\n\n"+g.responseText;alert(text)}})}
function djlst_post_attached_form(a){a.preventDefault();a=$(this);let c=a.val(),b=a.closest("form.listing-form"),e=b.attr("id"),d=$("#"+b.attr("related-listing"));var f=d.find(".row-container.selected"),g=a.attr("confirm-msg");if(g){var h=a.attr("confirm-msg-nb-items");h=parseInt(h);isNaN(h)&&(h=0);g=g.replace("{nb_items}",f.length);var l=d.attr("nb-rows");l||(l="");g=g.replace("{nb_all_items}",l);if(f.length>=h&&!confirm(g))return}g=f.map(function(){return $(this).attr("data-pk")}).get().join(",");
f=d.attr("ajax_url");h=d.attr("id");l=d.attr("listing-suffix");let m=a.attr("listing-target");m||(m="#"+h);let n=a.attr("listing-part");n||(n="all");a={listing_id:h,listing_suffix:l,listing_part:n,action:"attached_form",action_button:c,selected_pks:g,serialized_data:a.closest("form").serialize()};update_csrf_token();$.ajax({type:"POST",url:f,data:a,success:function(k){let p;k.listing&&d.replaceWith(k.listing);k.attached_form&&(b.replaceWith(k.attached_form),djlst_selection_changed_hook(d));k.object_pk&&
(p=$("#"+e),p.find('input[name="object_pk"]').val(k.object_pk));$(document).trigger("djlst_ajax_attached_form_loaded",{listing:m,form:p,response:k})},error:function(k){text="An error occured.\n\nIndications :\n\n"+k.responseText;alert(text)}})}var djlst_last_selected_rows_container=null,djlst_last_selected_index=null;
...
$(document).ready(function(){$(".filters-form .advanced-button").click(function(){$(this).closest(".filters-form").find(".filters-layout-advanced").slideToggle();$(this).find(".button-icon.up").toggle();$(this).find(".button-icon.down").toggle()});var a=!1;$("form.listing-form").submit(function(){$(this).find("input[name]").filter(function(){return!this.value}).prop("name","")});$(document.body).on("click","div.django-listing-ajax button.listing-nav",djlst_post_action_button);$(document.body).on("click",
"div.django-listing-ajax a.listing-nav",djlst_load_listing_href);$(document.body).on("change","div.django-listing-ajax select.listing-nav",djlst_load_listing_val);$("div.django-listing-selecting td.col-selection_checkbox").length?($(document.body).on("click","div.django-listing-selecting.selection_multiple .row-selector td.col-selection_checkbox",djlst_multiple_row_select),$(document.body).on("click","div.django-listing-selecting.selection_unique .row-selector td.col-selection_checkbox",djlst_unique_row_select)):
($(document.body).on("click","div.django-listing-selecting.selection_multiple .row-selector",djlst_multiple_row_select),$(document.body).on("click","div.django-listing-selecting.selection_unique .row-selector",djlst_unique_row_select));$(document.body).on("click","div.django-listing-selecting .selection-overlay.hover",djlst_activate_selection);$(document.body).on("click","[listing-action='select-all']",djlst_select_all);$(document.body).on("click","[listing-action='unselect-all']",djlst_unselect_all);
$(document.body).on("click","[listing-action='invert-selection']",djlst_invert_selection);$(document.body).on("click",".listing-selection-menu .listing-menu-close",djlst_deactivate_selection);$(document.body).on("click","li.action-item.view-object-popup a",djlst_view_object_popup);$(document.body).on("click",".button-action-group-by",function(){$(this).closest(".django-listing-container").find(".group-by-container").slideToggle(200)});$(document.body).on("click","form.django-listing-ajax.attached-form button[name='action_button']",
djlst_post_attached_form);$(document.body).on("click",".btn.gb-filter",function(){$(this).addClass("visited")});$(".django-listing-container").each(function(){djlst_selection_changed_hook($(this))});$('[data-toggle="popover"]').popover();var c=0;$(".dropzone").on("dragenter",function(){c++;$(this).addClass("drag-over")});$(".dropzone").bind("dragleave",function(){c--;0===c&&$(this).removeClass("drag-over")});$(".dropzone").bind("drop",function(){c=0;$(this).removeClass("drag-over")});$(".submit-action-form").on("click",
function(){var b=$(this).val(),e=$(this).closest(".django-listing-container").find(".action-form");e.find(".action-hidden-value").val(b);e.submit()});$(document).on("select2:open",function(b){b=$(b.target).parent().find(".select2-selection").attr("aria-owns");document.querySelector('input[aria-controls="'+b+'"]')&&(document.querySelector('input[aria-controls="'+b+'"]').focus(),a=!0)});$("form").keyup(function(b){9==b.keyCode&&(b=$(b.target),b.hasClass("select2-selection")?(a||b.closest(".select2").siblings("select").first().select2("open"),
a=!0):a=!1)});$(".file-generation-button").on("click",function(){var b=$(this).siblings("select");if(b.length&&!b.val())return(b=$(this).data("empty-select-msg"))||(b="Please select a value !"),alert(b),!1;Cookies.set("file_generation","working",{expires:1});$(this).closest("div.django-listing-ajax").addClass("spinning").removeClass("done");djlst_follow_file_generation()});djlst_listing_on_load()});
