...
function djlst_post_attached_form(a){a.preventDefault();a=$(this);let b=a.val(),c=a.closest("form.listing-form"),d=c.attr("id"),e=$("#"+c.attr("related-listing"));var f=e.find(".row-container.selected");if("update_all"==b||"update"==b&&1<f.length){var g=c.find("input:checkbox:visible").length;0==g&&(djlst_show_mass_op_cbs(c),djlst_clear_form(c));if(0==c.find("input:checkbox:checked").length||0==g){setTimeout(function(){alert(use_mass_cb_msg)},50);return}}"clear"==b&&djlst_listing_unselect_all(e);
if(g=a.attr("confirm-msg")){var h=a.attr("confirm-msg-nb-items");h=parseInt(h);isNaN(h)&&(h=0);g=g.replace("{nb_items}",f.length);var l=e.attr("nb-rows");l||(l="");g=g.replace("{nb_all_items}",l);if(f.length>=h&&!confirm(g))return}g=f.map(function(){return $(this).attr("data-pk")}).get().join(",");f=e.attr("ajax_url");h=e.attr("id");l=e.attr("listing-suffix");let m=a.attr("listing-target");m||(m="#"+h);let n=a.attr("listing-part");n||(n="all");a={listing_id:h,listing_suffix:l,listing_part:n,action:"attached_form",
