...
function djlst_get_form_data_advanced(a){const b=new FormData(a),c={};for(const [d,e]of b.entries())if(void 0===c[d]){const f=b.getAll(d);c[d]=1<f.length?f:e}a=Array.from(a.querySelectorAll('input[type="checkbox"][name]')).reduce((d,e)=>{let f;(d[f=e.name]||(d[f]=[])).push(e);return d},{});Object.entries(a).forEach(([d,e])=>{if(1===e.length){var f=e[0];void 0===c[d]&&(c[d]=f.checked?f.value||!0:!1)}else f=e.filter(g=>g.checked).map(g=>g.value||"on"),e=e.filter(g=>!g.checked).map(g=>g.value||"on"),
c[d]=f,c[`${d}__unchecked`]=e});return c}function djlst_add_filter_request_data(a,b,c){var d=a.attr("id");d=$(`form[listing-id=${d}]`);if(d.length){const e=d.attr("method");void 0!==e&&"post"==e.toLowerCase()&&(d=djlst_get_form_data_advanced(d[0]),c=Object.assign({},c,d))}0==b.closest(".toolbar_item.variationstoolbaritem").length&&(b=a.find(".group-by-select"),b.length&&(c.gb_cols=b.val().join(",")),a=a.find(".annotation-select"),a.length&&(c.gb_annotate_cols=a.val().join(",")));return c}
...
function djlst_load_listing_url(a,b,c){null===b&&(b=djlst_get_requested_url(a));var d=djlst_extract_params_from_url(b,["variation"]);0<Object.keys(d).length&&(c=c?Object.assign({},c,d):d);$("div.django-listing-ajax").length||(window.location.href=b);const e=a.closest("div.django-listing-ajax");e.addClass("spinning");d=$(e).attr("id");const f=$(`form[listing-id=${d}]`);var g=$(e).attr("listing-suffix");let k=a.attr("listing-target");k||(k="#"+d);let h=a.attr("listing-part");h||(h="all");d={listing_id:d,
listing_suffix:g,listing_part:h};(g=a.attr("action"))&&(d.action=g);d=djlst_add_filter_request_data(e,a,d);c&&(d=Object.assign({},d,c));$(document).trigger("djlst_before_load_listing_url",{listing:k,url:b,payload:d});update_csrf_token();$.ajax({type:"POST",url:b,data:d,traditional:!0,success:function(l){l.filters_form&&(f.replaceWith(l.filters_form),e.removeClass("spinning"));l.listing&&(e.replaceWith(l.listing),djlst_listing_on_load(),$(document).trigger("djlst_ajax_loaded",{listing_target:k,response:l.listing}))},
error:function(l){text="An error occured.\n\nIndications :\n\n"+l.responseText;alert(text)}});$(document).trigger("djlst_after_load_listing_url");return!1}function djlst_load_listing_href(){return djlst_load_listing_url($(this),$(this).attr("href"))}function djlst_load_listing_val(){var a=$(this).val(),b=$(this).attr("name"),c=$(this).closest("div.django-listing-ajax").attr("ajax_url");a=djlst_replaceUrlParam(c,b,a);return djlst_load_listing_url($(this),a)}
function djlst_post_action_button(a){a.preventDefault();a=$(this);const b=a.closest("div.django-listing-ajax"),c=b.attr("ajax_url");b.addClass("spinning");var d=b.attr("id");const e=$(`form[listing-id=${d}]`),f=$("#"+b.attr("attached-form-id")),g=b.attr("listing-suffix");let k=a.attr("listing-target");k||(k="#"+d);let h=a.attr("listing-part");h||(h="all");d={listing_id:d,listing_suffix:g,listing_part:h,serialized_data:a.closest("form").serialize()};d[$(this).attr("name")]=$(this).val();d=djlst_add_filter_request_data(b,
a,d);update_csrf_token();$(document).trigger("djlst_before_post_action_button",{listing:k,attached_form:f,payload:d});$.ajax({type:"POST",url:c,data:d,traditional:!0,success:function(l){l.filters_form&&(e.replaceWith(l.filters_form),b.removeClass("spinning"));l.listing&&(b.replaceWith(l.listing),djlst_listing_on_load(),$(document).trigger("djlst_ajax_loaded",{listing_target:k,response:l.listing}))},error:function(l){text="An error occured.\n\nIndications :\n\n"+l.responseText;alert(text)}});$(document).trigger("djlst_after_post_action_button",
{listing:k,attached_form:f,payload:d})}var djlst_mass_op_cbs_displayed=!1;function djlst_show_mass_op_cbs(a){djlst_mass_op_cbs_displayed=!1;a.find(".mass-op-cb").prop("checked",!1).show();a.addClass("mass-op-cbs-displayed");setTimeout(function(){djlst_mass_op_cbs_displayed=!0},100)}function djlst_hide_mass_op_cbs(a){a.find(".mass-op-cb").hide();a.removeClass("mass-op-cbs-displayed");djlst_mass_op_cbs_displayed&&a.find(".form-buttons button.clear")[0].click();djlst_mass_op_cbs_displayed=!1}
function djlst_copy_f_fields_to_attached_form(a){var b=a.attr("related-listing");if(b){var c=$('form[listing-id="'+b+'"]').first();if(0!==c.length){var d=new Set;c.find(':input[name^="f_"]').each(function(){var e=$(this).attr("name");if(e&&!d.has(e)){d.add(e);var f=a.find(':input[name="'+e.replace(/([\\[\\].])/g,"\\$1")+'"]');if(0!==f.length){e=c.find(':input[name="'+e.replace(/([\\[\\].])/g,"\\$1")+'"]');var g=e.first(),k=(g.attr("type")||"").toLowerCase(),h=[];"select"===(g[0]&&g[0].tagName||"").toLowerCase()?
(e=e.val(),Array.isArray(e)?h=e.filter(function(n){return null!=n}):null!=e&&(h=[e])):"checkbox"===k?e.filter(":checked").each(function(){var n=$(this).val();h.push(null==n||""===n?"on":n)}):"radio"===k?(e=e.filter(":checked").first(),e.length&&(h=[e.val()])):(e=g.val(),null!=e&&(h=[e]));e=f.first();g=(e.attr("type")||"").toLowerCase();if("select"===(e[0]&&e[0].tagName||"").toLowerCase())e.prop("multiple")?f.val(h):f.val(h.length?h[0]:""),f.trigger("change");else if("checkbox"===g){var l=new Set(h.map(String));
f.each(function(){var n=$(this),p=n.val();if(null==p||""===p)p="on";n.prop("checked",l.has(String(p)))}).trigger("change")}else if("radio"===g){var m=h.length?String(h[0]):null;f.each(function(){var n=$(this);n.prop("checked",null!==m&&String(n.val())===m)}).trigger("change")}else f.val(h.length?h[0]:"").trigger("change")}}})}}}
function djlst_post_attached_form(a){const b=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a.preventDefault();const c=$(b),d=c.closest("form").find(".form-fields"),e=c.val(),f=c.closest("form.listing-form"),g=f.closest(".attached-form-container");let k=f.attr("id"),h=$("#"+f.attr("related-listing"));var l=h.find(".row-container.selected");!c.hasClass("flip")&&g.hasClass("flipped")&&(d.addClass("flip-out"),f.hasClass("animate")&&(yield djlst_sleep(300)));var m=c.hasClass("mass-cb-required");
if("update_all"==e||m||"update"==e&&1<l.length)if(m=f.find("input.mass-op-cb:visible").length,0==m&&(djlst_show_mass_op_cbs(f),djlst_clear_form(f)),0==f.find("input.mass-op-cb:checked").length||0==m){setTimeout(function(){alert(use_mass_cb_msg)},50);return}"clear"==e&&djlst_listing_unselect_all(h);if(m=c.attr("confirm-msg")){var n=c.attr("confirm-msg-nb-items");n=parseInt(n);isNaN(n)&&(n=0);m=m.replace("{nb_items}",l.length);var p=h.attr("nb-rows");p||(p="");m=m.replace("{nb_all_items}",p);if(l.length>=
n&&!confirm(m))return}djlst_copy_f_fields_to_attached_form(f);c.hasClass("flip")&&(d.addClass("flip-out"),g.addClass("flipped"),f.hasClass("animate")&&(yield djlst_sleep(300)));m=l.map(function(){return $(this).attr("data-pk")}).get().join(",");l=h.attr("ajax_url");let u=h.attr("id");n=h.attr("listing-suffix");let t=c.attr("listing-target");t||(t="#"+u);(p=c.attr("listing-part"))||(p="all");m={listing_id:u,listing_suffix:n,listing_part:p,action:"attached_form",action_button:e,selected_pks:m,serialized_data:c.closest("form").serialize()};
m=djlst_add_filter_request_data(h,c,m);$(document).trigger("djlst_before_attached_form_post",{listing:t,form:f,payload:m,nav_obj:c});update_csrf_token();$.ajax({type:"POST",url:l,data:m,traditional:!0,success:function(q){let r;q.listing&&(h.replaceWith(q.listing),h=$("#"+u),djlst_listing_on_load());q.attached_form&&(f.replaceWith(q.attached_form),djlst_selection_changed_hook(h),g.hasClass("flipped")&&(r=$("#"+k),r.find(".mass-op-cb").hide(),r.removeClass("mass-op-cbs-displayed"),djlst_mass_op_cbs_displayed=
!1));q.object_pk&&(r=$("#"+k),r.find('input[name="object_pk"]').val(q.object_pk));r=$("#"+k);$(document).trigger("djlst_ajax_attached_form_loaded",{listing:t,form:r,response:q,action_button:e});c.hasClass("flip")&&""!==q.layout_name||!g.hasClass("flipped")||setTimeout(()=>{g.removeClass("flipped");d.removeClass("flip-out")},300)},error:function(q){text="An error occured.\n\nIndications :\n\n"+q.responseText;alert(text)}});$(document).trigger("djlst_after_attached_form_post",{listing:t,form:f,payload:m,
...
function djlst_fill_attached_form(a){let b=a.find('input[name="object_pk"]').val();if(b){var c=a.attr("id");c=$('.django-listing-container[attached-form-id="'+c+'"]').find('.row-container[data-pk="'+b+'"]');c.length&&(c=c.attr("data-serialized-object"))&&(c=decodeURIComponent(escape(atob(c))),c=JSON.parse(c),djlst_fill_form(a,c,b))}}
function djlst_set_form_input(a,b){if(a.is(":input"))if(a.is("input[type='radio']"))a.filter("[value='"+b+"']").prop("checked",!0);else if(a.is("input[type='checkbox']"))a.prop("checked",b);else if(a.is("select"))if(Array.isArray(b)&&b.every(Array.isArray)){a.empty();for(const [c,d]of b)a.append($("<option>",{value:c,text:d,selected:!0}));a.trigger("change")}else{let c=b;Array.isArray(b)&&(c=b[1],b=b[0]);"boolean"===typeof b&&(b=b?"True":"False");b||(b="");0===a.find("option[value='"+b+"']").length&&
(a.hasClass("select2-hidden-accessible")&&a.empty(),a.append($("<option>",{value:b,text:c})));a.val(b);a.hasClass("select2-hidden-accessible")&&a.trigger("change")}else a.val(b)}
function djlst_fill_form(a,b,c){a.find('input[name="object_pk"]').val(c);a.find("input, select, textarea").each(function(){let d=$(this);var e=d.attr("name");if(void 0!==b.formfields&&e in b.formfields)e=b.formfields[e];else if(void 0!==b.fields&&e in b.fields)e=b.fields[e];else{if("hidden"===this.type||void 0!==b.data&&void 0!==b.data.no_autofill&&b.data.no_autofill.includes(e))return!0;e=""}djlst_set_form_input(d,e)});$(document).trigger("djlst_form_filled",{form:a})}
