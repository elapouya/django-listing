...
function djlst_load_listing_url(a,b){null===b&&(b=djlst_get_requested_url(a));$("div.django-listing-ajax").length||(window.location.href=b);const c=a.closest("div.django-listing-ajax");c.addClass("spinning");var d=$(c).attr("id");const e=$(c).attr("listing-suffix");let f=a.attr("listing-target");f||(f="#"+d);let g=a.attr("listing-part");g||(g="all");d={listing_id:d,listing_suffix:e,listing_part:g};d=djlst_add_filter_request_data(c,a,d);update_csrf_token();$.ajax({type:"POST",url:b,data:d,success:function(h){$(f).replaceWith(h);
djlst_listing_on_load();$(document).trigger("djlst_ajax_loaded",{listing_target:f,response:h})},error:function(h){text="An error occured.\n\nIndications :\n\n"+h.responseText;alert(text)}});return!1}function djlst_load_listing_href(){return djlst_load_listing_url($(this),$(this).attr("href"))}
function djlst_load_listing_val(){var a=$(this).val(),b=$(this).attr("name"),c=$(this).closest("div.django-listing-ajax").attr("ajax_url");a=djlst_replaceUrlParam(c,b,a);return djlst_load_listing_url($(this),a)}
function djlst_post_action_button(a){a.preventDefault();a=$(this);var b=a.closest("div.django-listing-ajax"),c=b.attr("ajax_url");b.addClass("spinning");var d=b.attr("id"),e=b.attr("listing-suffix"),f=a.attr("listing-target");f||(f="#"+d);var g=a.attr("listing-part");g||(g="all");d={listing_id:d,listing_suffix:e,listing_part:g,serialized_data:a.closest("form").serialize()};d[$(this).attr("name")]=$(this).val();d=djlst_add_filter_request_data(b,a,d);update_csrf_token();$.ajax({type:"POST",url:c,data:d,
success:function(h){$(f).replaceWith(h);djlst_listing_on_load();$(document).trigger("djlst_ajax_loaded",{listing_target:f,response:h})},error:function(h){text="An error occured.\n\nIndications :\n\n"+h.responseText;alert(text)}})}var djlst_mass_op_cbs_displayed=!1;function djlst_show_mass_op_cbs(a){djlst_mass_op_cbs_displayed=!1;a.find(".mass-op-cb").prop("checked",!1).show();a.addClass("mass-op-cbs-displayed");setTimeout(function(){djlst_mass_op_cbs_displayed=!0},100)}
function djlst_hide_mass_op_cbs(a){a.find(".mass-op-cb").hide();a.removeClass("mass-op-cbs-displayed");djlst_mass_op_cbs_displayed&&a.find(".form-buttons button.clear")[0].click();djlst_mass_op_cbs_displayed=!1}
...
50);return}}"clear"==b&&djlst_listing_unselect_all(e);if(g=a.attr("confirm-msg")){var h=a.attr("confirm-msg-nb-items");h=parseInt(h);isNaN(h)&&(h=0);g=g.replace("{nb_items}",f.length);var l=e.attr("nb-rows");l||(l="");g=g.replace("{nb_all_items}",l);if(f.length>=h&&!confirm(g))return}g=f.map(function(){return $(this).attr("data-pk")}).get().join(",");f=e.attr("ajax_url");h=e.attr("id");l=e.attr("listing-suffix");let m=a.attr("listing-target");m||(m="#"+h);let n=a.attr("listing-part");n||(n="all");
b={listing_id:h,listing_suffix:l,listing_part:n,action:"attached_form",action_button:b,selected_pks:g,serialized_data:a.closest("form").serialize()};b=djlst_add_filter_request_data(e,a,b);update_csrf_token();$.ajax({type:"POST",url:f,data:b,success:function(k){let p;k.listing&&e.replaceWith(k.listing);k.attached_form&&(c.replaceWith(k.attached_form),djlst_selection_changed_hook(e));k.object_pk&&(p=$("#"+d),p.find('input[name="object_pk"]').val(k.object_pk));$(document).trigger("djlst_ajax_attached_form_loaded",
{listing:m,form:p,response:k})},error:function(k){text="An error occured.\n\nIndications :\n\n"+k.responseText;alert(text)}})}var djlst_last_selected_rows_container=null,djlst_last_selected_index=null;
...
function djlst_get_requested_url(a){a=a.closest("div.django-listing-container").find("input[name='requested_url']");return a.length?a.val():window.location.href}function djlst_reload_listing_from_form(a){a=$(a).closest(".listing-form").attr("listing-id");a=$("#"+a);const b=djlst_get_requested_url(a);djlst_load_listing_url(a,b)}
...
function djlst_reset_form_fields(a){a[0].reset();a.find("select").each(function(){$(this).hasClass("select2-hidden-accessible")&&$(this).val(null).trigger("change")})}
