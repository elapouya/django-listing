...
function djlst_load_listing_url(a,b){var c=a.closest("div.django-listing-ajax");c.addClass("spinning");var d=$(c).attr("id"),f=$(c).attr("listing-suffix"),e=a.attr("listing-target");e||(e="#"+d);(a=a.attr("listing-part"))||(a="all");update_csrf_token();$(e).load(b,{listing_id:d,listing_suffix:f,listing_part:a},function(g,h,k){"error"==h?(c.removeClass("spinning"),alert(g)):$(this).children(":first").unwrap();$(document).trigger("djlst_ajax_loaded",[e])});return!1}
...
d.find(".action-hidden-value").val(c);d.submit()});$(document).on("select2:open",function(c){c=$(c.target).parent().find(".select2-selection").attr("aria-owns");document.querySelector('input[aria-controls="'+c+'"]').focus();a=!0});$("form").keyup(function(c){9==c.keyCode&&(c=$(c.target),c.hasClass("select2-selection")?(a||c.closest(".select2").siblings("select").first().select2("open"),a=!0):a=!1)});$("select.group-by-dual-listbox").each(function(){let c=this;new DualListbox(this,{addButtonText:">",
removeButtonText:"<",addAllButtonText:">>",removeAllButtonText:"<<",availableTitle:$(this).attr("available-title")||"Available columns",selectedTitle:$(this).attr("selected-title")||"Selected columns"});$(this).parent().find(".apply-group-by").on("click",function(){var d=$(c).val().join(",");d=djlst_replaceUrlParam(window.location.href,"gb_cols",d);window.location.href=d});$(this).parent().find(".remove-group-by").on("click",function(){let d=djlst_replaceUrlParam(window.location.href,"gb_cols","");
